...
using MiniSaaS.Application.Tenants.Validators;
...
public sealed class TenantService : ITenantService
{
        ...
        CancellationToken cancellationToken = default)
    {
        var validationResult =
            await _createTenantValidator.ValidateAsync(
                request,
                cancellationToken);
        ...
        var slugExists =
            await tenantRepository.ExistsAsync(
        ...
        {
            _logger.LogWarning(
                "Tenant creation conflict. Slug {Slug} already exists.",
                request.Slug);
        ...
        _logger.LogInformation(
            "Tenant {TenantId} created successfully with slug {Slug}.",
            tenant.Id,
            tenant.Slug);
